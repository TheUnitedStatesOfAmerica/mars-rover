...
fn test_parse_position() {
  let input = "1 5 E";
  let output: (Position, Direction) = rover::parse_position(input).expect("could not parse location and direction");
...
fn test_rover_init() {
  ...
  let rover1_initial_data: (Position, Direction) = rover::parse_position(lines.next().unwrap()).expect("could not parse location and direction");

  let mut rover1 = Rover::new(rover1_initial_data.0, rover1_initial_data.1, world);
  rover1.run_to_end(lines.next().unwrap()).expect("could not complete moves");

  let rover2_initial_data: (Position, Direction) = rover::parse_position(lines.next().unwrap()).expect("could not parse location and direction");
  let mut rover2 = Rover::new(rover2_initial_data.0, rover2_initial_data.1, world);
  rover2.run_to_end(lines.next().unwrap()).expect("could not complete moves");
